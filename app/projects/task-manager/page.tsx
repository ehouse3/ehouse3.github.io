    ...
    <div>
      ...
      <Section title="Challenges Faced" data-desc="challenges">
        <SubSection title="RESTful API">
          <p>
            The RESTful API proved challenged for a number of reasons, debugging
            was difficult and tedious, interacting with JSON was error prone,
            and finally consistency across all endpoints.
          </p>
          <p>
            ...
            through undiscovered. I needed to stop these errors in their tracks.
            I had experience designing a database and the relations that needed
            to exist for it to function, but interfacing with the backend was
            difficult. This API felt weaker than it needed to be. A major bug I
            came across was because of JSON types and how it worked with
            ...
            caused data to be lost in transit. Although now it feels embarrasing
            that such a small error caused so much headache, at the time it was
          ...
          <p>
            ...
            with how typescript types work, which will make this a must have in
            the future. Lucrative API testing would also have proven useful, and
            would have gone a long way to reducing debugging time. I didn&apos;t
            fall for the pitfalls that I imagined most juniors might have fallen
            for in making an API for the first time, but I learned a lot from
        ...
        <SubSection title="Spring Boot">
          <p>
            ...
            for OOP, and expanding the CRUD system, it was straight forward. The
            core difficulty lied in using Spring Boot, and ensuring the
            annotations worked the way I thought they would work. Spring Boot is
            a very abstracted and annotation heavy package compared to what I
            have used in the past. In the beginning, there was a lot happening
            under the hood that I was unable to see, and thus caused issues. But
            I quickly came to understand why each annotation was there and the
            problem that was fixed because of it, which assisted with
            organization in the end. Its annotations for reducing boilerplate
            code helped speed up developement as well!
          </p>
        </SubSection>
        ...
        <SubSection title="Filter and Middleware">
          <p>
            ...
            the JWT tokens, or JSON errors, or even API issues. The filter
            itself generally was easy to design. It needed to be lightweight
            because it was running on every API call, but also robust to catch
            as many errors as it can. I found keeping the filter simple proved
            to be an effective approach, and to throw a lot of errors
          </p>
        </SubSection>
        ...
        <SubSection title="Authentication Context">
          <p>
            ...
            be able to branch most of the application. The issues that I
            encountered for this task wasn&apos;t because it was particularly
            hard, but because I had high expections for it. It would be user and
            token context for the entire app, both public and authenticated
            endpoints, and needed functions to handle changing data as well.
            Finally, the user context being a major point in the pipeline, it
            needed to not suppress errors when they occured.
          </p>
        </SubSection>
      </Section>
      ...
      >
        <SubSection title="Next.js & React">
          ...
          <p>
            ...
            on every request redirected tokenless users to login pages, and let
            the backend verify tokens through the filter. Extensive components
            usage, and the Next.js Layout helped create reusable and organized
            elements. The components especially helped for elements like forms,
            and displaying projects themselves. The client side app router
            simplified the developemnt, and paired with dynamic routes made
            creating pages for specific project pages great.
          </p>
        </SubSection>
        ...
        <SubSection title="Rest API Pipeline">
          <p>
            ...
            frontend needed API functions to call the backend API. Essentially,
            each function that existed in the backend had a function that would
            call it in the front end, like{" "}
            <code>getUserByEmail(), getProjectByUserId()</code>. The front-end
            uses Axios as its library of choice, and this section was integrated
            with multiple other systems as well. For instance, the client had to
            attach the JWT in the cookies to every request for the backend to
            ...
            but was similar to the front end in it was the system that other
            systems were integrated with. Like the filter.
          </p>
        </SubSection>

        <SubSection title="Auth Context">
          <p>
            As I talked about in the Challenges section, I had higher expections
            for the auth context but I also have never used something like this
            before. The Authentication Context is a wrapper for the front end
            pages, that can be accessed at any page for user data. This prevents
            prop drilling among other things. It provides functions like{" "}
          ...
          <p>
            ...
            initializer for the auth to assign pre-existing user cookies created
            a smoother user experience as well. The Auth now still works if the
            user leaves and comes back!
          </p>
          <p>
            I wanted to experiement with this as well, and decided try out some
            features that expand on what many simple guides online have about
            authContext. I created an initializer. I created a simple reusable
            function for assigning the values into cookies, and I created a
            function to refresh the user. A major sepparation I tried to
            introduce was a sepparate provider for authenticated pages and a
            provider for unauthenticated pages. There were multiple ideas behind
            this implementation, the login page and register page don&apos;t
            need logout functions, and the protected dashboard page doesn&apos;t
            need login functions. I noticed this when I asked a question while
            ...
            for mainly the context page, because it added another layer to keep
        ...
        <SubSection title="Authentication and Security Pipeline">
          <p>
            ...
            closely with enterprise solutions, and each at different sections of
            the data pipeline. The user starts at the front end, where the first
            layer exists. Middleware will redirect the user off of authenticated
            pages only if it&apos;s missing a token. This isn&apos;t perfect
            security because the authenitcation itself doesn&apos;t happen here,
            but it is not designed to. Similar to the middleware, the context
            will throw a lot of errors if it is not in formatted correctly.
          </p>
          <p>
            ...
            create its context through spring boot and enusre authorization. For
            an added bonus, passwords are never sent out of the database and
            backend. Only brought in, and then validated. For password security,
            the passwords are stored hashed, and simple requirements for length
            and complexity are in place. I considered implementing password
            salting, but ultimately found it unnecessary for learning since I
            already know it.
          </p>
        </SubSection>
      </Section>
      ...
      <Section title="Design Details - Back End" data-dest="back end details">
        <SubSection title="Spring Boot & Statelessness">
          <p>
            ...
            so multiple companies and industries use it. Configuring the setup,
          ...
          <p>
            ...
            to generally the single qpi request being sent and responded.
            Getting started was difficult, but once the CRUD system was
            established, it was easier to exapnd upon. It was difficult ensuring
            ...
            app uses DTO&apos;s (Data Transfer Objects), to provide a predefined
            schema of data. This helped organize the app too, because it provded
            a schema and easily showed what was needed at any function.
          </p>
          <p>
            ...
            implement an authentication controller and service file sepparately
            from the others, to fully establish the registering and logging in
            ...
            orgainzed well into multiple layers.
          </p>
        </SubSection>
        ...
        <SubSection title="Postgresql Database">
          <p>
            ...
            was to ensure the database stayed unidiretional. The architecturaly
            ...
            user by their Id, with eagerly fetched projects) have to be
            specially created.
          </p>
          <p>
            This project did not start using postgreSQL either. Partway through,
            I decided to migrate from SQL to postgrSQL, because I wanted to
            experiment and learn with popular querly languages, and Spring Boot
        ...
        <SubSection title="Docker">
          <p>
            ...
            reusalbe containers using images. As before, Docker is a tool that
            many industries use to create software, so I wanted to learn more
            about it. I&apos;ve toched upon it in the past, but never wrote my
            own images. I new I wanted to use it because it aligned with my
            goals, but also because I actually depended upon it as well. I use
            two computers and often times develope this app on one for a time
            and then the other for a time. Regardless of what I added, docker
            ensured the underlying software ran on both.
          </p>
        </SubSection>

        <SubSection title="Testing">
          <p>
            Following the goals for following industry methods and build tools,
            and working with the features of the frameworks too. Testing is an
            integral part of writing software, because it accounts for errors to
            be fixed. Although in the moment they are frustrating because it
            seems they cause errors and slow down production, in the long run it
            actually solves problems before they show up which speeds up
            production. Spring boot has libraries for testing, with built in
            annotations which I utilized extensively. I implemented both
            integration and unit testing for the Authentication pipeline of the
            app. They tested both login failures with many possible reasons for
            failures, and a full registration run through as well. Not only did
            these catch incorrectly named HTTP errors, but also an ID assignment
            bug as well. Next app I make, im starting testing on many of the
            layers i worked in, and am starting it earlier. I implemented
            testing after the fact, so it was a lot less useful than it could
            have been, despite the fact that making the testing did resolve some
            errors and ensure my error handling was more up to par with names.
          </p>
        </SubSection>
        ...
        <SubSection title="AI Use">
          <p>
            ...
            experience trouble shooting problems. AI usage had multiple benifits
            and drawbacks during developement. One major benifit was using AI
            was making the skeleton for this app. I found it difficult to know
            where to even begin, and AI helped walk through that phase. Some
            examples would be configuration of the database and application
            properties, setting up the docker images, or building generic
            skeleton files for different layers. It built the bare skeleton for
            the controller, and I added the service layer, and login. Some
            issues I ran into when using AI was how often I was fighting against
            it when it tried to solve actual errors or coding problems. A good
            example was designing the relations for the database. Admitidly,
            this was partially my fault because I did not go into designing the
            database with a rigid enough schema. To solve problems about
            retrieving data and setting up data relationships, it oftentimes
            wanted to make the data bidirectional. 9 times out of 10 this made
            the issue worse and caused major errors elsewhere. I never used AI
            as a crutch during this project, but prioritized using it as a tool
            to bounce ideas off of, learn from, and as a starting point.
          </p>
        </SubSection>
      </Section>
    </div>
